...
export default {
    ...
    execute: async({ hisoka, m, quoted }) => {
        ...
        if (/image|video|sticker/.test(quoted.mime)) {
            ...
            hisoka.sendMessage(m.from, url, { asSticker: true, quoted: m, packName: `Hostage Bot :\nhttps://instagram.com/moreegih\n\nPada :\n${Func.tanggal(new Date())} Pukul ${Func.jam(new Date())} WIB\n\nOleh :\n${m.pushName}`, packPublish: global.Exif.packPublish })
        } else if (/(https?:\/\/.*\.(?:png|jpg|jpeg|webp|mov|mp4|webm))/i.test(m.text)) {
            hisoka.sendMessage(m.from, Func.isUrl(m.text)[0], { quoted: m, asSticker: true, packName: `Hostage Bot :\nhttps://instagram.com/moreegih\n\nPada :\n${Func.tanggal(new Date())} Pukul ${Func.jam(new Date())} WIB\n\nOleh :\n${m.pushName}`, packPublish: global.Exif.packPublish })
        } else {
